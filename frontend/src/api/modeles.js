...
export async function modifierModeleApi(id, modele) {
  const reponse = await fetchAuthentifie(`/modeles/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(modele),
  });
  const donnees = await reponse.json();
  if (!reponse.ok) throw new Error(donnees.erreur || 'Erreur lors de la modification du modèle.');
  return donnees;
}
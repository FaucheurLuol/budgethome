...
export async function obtenirMoiApi() {
  const reponse = await fetchAuthentifie('/auth/moi');
  const donnees = await reponse.json();
  if (!reponse.ok) throw new Error(donnees.erreur || 'Erreur.');
  return donnees;
}

export async function deconnexionApi() {
  const reponse = await fetchAuthentifie('/auth/deconnexion', { method: 'POST' });
  if (!reponse.ok) throw new Error('Erreur lors de la déconnexion.');
}
